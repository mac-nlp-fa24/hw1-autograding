...
for fn in os.listdir(test_folder):
    ...
    for n in [1, 2, 3, 4]:
        ...
        out = subprocess.run(mle_cmd.split(), capture_output=True)
        save(output_dir, out, mle_cmd, i)
        i += 1

        # laplace
        l_cmd = run_cmd.format(path + "/train.txt", path + "/test.txt", "laplace", n) 
        out = subprocess.run(l_cmd.split(), capture_output=True)
    ...
    for n in [3, 4]:
        ...
        for coeffs in coeffss[n]:
            ...
            out = subprocess.run(interp_cmd.split(), capture_output=True)
    ...
    for n in [3, 4]:
        ...
        for d in discounts:
            ...
            out = subprocess.run(bo_cmd.split(), capture_output=True)
